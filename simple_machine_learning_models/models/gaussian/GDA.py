...
class GDA:
    ...
    def train(self, train_x, train_y):
        ...
        for i in range(len(train_y)):
            if train_y[i] == 0:
                self.sigma += np.dot(np.transpose(np.asmatrix(train_x[i] - self.mu0)), np.asmatrix(train_x[i] - self.mu0))
            else:
                self.sigma += np.dot(np.transpose(np.asmatrix(train_x[i] - self.mu1)), np.asmatrix(train_x[i] - self.mu1))
